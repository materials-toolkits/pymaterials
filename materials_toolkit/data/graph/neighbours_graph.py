...
class NeighboursGraphBuilder:
    def __init__(
        ...
        idx = torch.arange(self.shapes.n, dtype=torch.long, device=self.shapes.device)
        ...
        self.edges = torch.empty((0, 5), dtype=torch.long, device=self.shapes.device)
    ...
    def blend_with_edges_(
        ...
        idx_cell = torch.div(idx, num_edges[batch_blended], rounding_mode="trunc")
    ...
    def gen_edges_range_(
        ...
        mask = (
            (cell_coords <= (-start[struct_idx])) | (start[struct_idx] <= cell_coords)
        ).any(dim=1)
    ...
    def push(self, start: Optional[torch.IntTensor], limit: torch.IntTensor = None):
        ...
        e_ij = self.pos[edges[:, 1]] + edges[:, 2:].float() - self.pos[edges[:, 0]]
        v_ij = torch.bmm(
    ...
    def tokenize(self, edges: torch.LongTensor) -> torch.LongTensor:
        ...
        return (
            (
                (
                    (edges[:, 0] * max_idx + edges[:, 1]) * diff_coords
                    + edges[:, 2]
                    - min_coords
                )
                * diff_coords
    ...
    def get_reverse_idx(
        self, edges: torch.LongTensor, tokens: torch.LongTensor
    ) -> Tuple[torch.LongTensor, torch.LongTensor]:
    ...
    def build(self):
        start = torch.full_like(self.cell[:, :, 0], fill_value=0, dtype=torch.int32)
        limit = torch.full_like(self.cell[:, :, 0], fill_value=1, dtype=torch.int32)
